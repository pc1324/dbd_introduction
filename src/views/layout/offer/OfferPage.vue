...
import { getOfferListService } from '@/api/offer'
import defaultAvatar from '@/assets/avatar.jpg'
import OfferDetailsDialog from './components/OfferDetailsDialog.vue'
...
watch(
  ...
  (newValue) => console.log('offerType发生了变化', newValue)
)
...
<template>
  <div class="offer-page">
    <div class="header">
      <div class="text">
        <h1 style="font-size: 42px">祭品介绍</h1>
        <p>
          在黎明杀机中，每名玩家都可以携带一个祭品进入游戏，祭品拥有改变游戏部分规则的能力，祭品为消耗品，在下面探索
          Dead by Daylight 的完整祭品名单。
        </p>
        <div class="search">
          <div class="search-item">
            <label for="name">搜索祭品名</label>
            <input
              type="text"
              id="name"
              v-model="params.name"
          ...
          <div class="search-item">
            <label for="identity">祭品种类</label>
            <!-- Vue3 => v-model 是 :modelValue 和 @update:modelValue 的简写 -->
            <select v-model="params.offerType" @change="search">
              <!-- label是展示给用户看的，value是提交给后台的值 -->
              <option label="全部" :value="null"></option>
              <option label="通用" value="0"></option>
              <option label="逃生者祭品" value="1"></option>
              <option label="杀手祭品" value="2"></option>
            </select>
          </div>
        </div>
        <div class="search">
          <div class="search-item">
            <label for="identity">祭品品质</label>
            <!-- Vue3 => v-model 是 :modelValue 和 @update:modelValue 的简写 -->
            <select v-model="params.quality" @change="search">
              <!-- label是展示给用户看的，value是提交给后台的值 -->
              <option label="全部" :value="null"></option>
              <option label="普通" value="普通"></option>
              <option label="精良" value="精良"></option>
              <option label="稀有" value="稀有"></option>
              <option label="非常稀有" value="非常稀有"></option>
              <option label="超级稀有" value="超级稀有"></option>
              <option label="活动" value="活动"></option>
            </select>
          </div>
        </div>
      </div>
    ...
    <!-- 弹窗组件 -->
    <OfferDetailsDialog ref="detailsDialog"></OfferDetailsDialog>
  </div>
</template>
