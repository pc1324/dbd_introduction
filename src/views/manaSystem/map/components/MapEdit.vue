...
import { getMapByIdService, addMapService, updateMapService } from '@/api/map'
...
const open = async (row) => {
  ...
  if (row.id) {
    ...
    // console.log('编辑')
    // 数据回显，先用id 查询数据
    const res = await getMapByIdService(row.id)
    // console.log(res)
...
<template>
  <el-drawer
    ...
    :title="formModel.id ? '编辑地图' : '新增地图'"
    direction="rtl"
    size="50%"
  >
    <!-- 表单 -->
    <el-form ref="form" :rules="rules" :model="formModel" label-width="100px">
      <el-form-item label="地图名" prop="name">
        <el-input
          v-model="formModel.name"
          placeholder="请输入地图名"
        ></el-input>
      </el-form-item>
      <el-form-item label="地图系列" prop="series">
        <el-input
