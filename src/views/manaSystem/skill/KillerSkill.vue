<script setup>
import KillerEdit from './components/KillerSkillEdit.vue'
import { ref, watch } from 'vue'
import { Edit, Delete } from '@element-plus/icons-vue'
import {
  getKillerSkillListService,
  getKillerByIdService,
  deleteKillerSkillService
} from '@/api/killer'
import OwnerSelect from './components/OwnerSelect.vue'
import userAvator from '@/assets/default.png'

const listData = ref([]) // 杀手技能列表数据
...
const getKillerSkillList = async () => {
  ...
  listData.value.forEach(async (item) => {
    ...
    if (res.data.data !== null) {
      item.killerIdentity = res.data.data.identity
    } else {
      item.killerIdentity = '通用技能'
    }
  })
...
const handleDelete = async (id) => {
  console.log(id)
  // 提示框
  await ElMessageBox.confirm('你确认要删除所选杀手技能吗？', '温馨提示', {
    type: 'warning',
    confirmButtonText: '确认',
    cancelButtonText: '取消'
  })
  await deleteKillerSkillService(id)
...
<template>
  <div class="killer-skill-list">
    <el-card style="max-width: 100vw">
      <template #header>
        <div class="card-header">
          <span>杀手技能列表</span>
          <div class="btns">
            <el-button type="primary" @click="onAddKiller"
              >添加杀手技能</el-button
            >
      ...
      <el-form inline>
        ...
        <el-form-item label="杀手技能名">
          <el-input v-model="params.name" placeholder="请输入杀手技能名" />
        </el-form-item>
      ...
      >
        ...
        <el-table-column prop="killerIdentity" label="所有者" width="180" />
