...
const open = async (row) => {
  ...
  if (row.id) {
    ...
    // console.log('编辑')
    // 数据回显，先用id 查询数据
    const res = await getOfferByIdService(row.id)
    // console.log(res)
...
<template>
  <el-drawer
    ...
    :title="formModel.id ? '编辑祭品' : '新增祭品'"
    direction="rtl"
    size="50%"
  >
    <!-- 表单 -->
    <el-form ref="form" :rules="rules" :model="formModel" label-width="100px">
      <el-form-item label="祭品名" prop="name">
        <el-input
          v-model="formModel.name"
          placeholder="请输入祭品名"
        ></el-input>
      </el-form-item>
      <el-form-item label="祭品品质" prop="quality">
        <el-select :style="{ width: '220px' }" v-model="formModel.quality">
      ...
      <el-form-item label="祭品种类" prop="offerType">
        <el-select :style="{ width: '220px' }" v-model="formModel.offerType">
          <!-- label是展示给用户看的，value是提交给后台的值 -->
      ...
      <el-form-item label="祭品介绍" prop="description">
        <div class="editor">
          <quill-editor
            ref="editorRef"
